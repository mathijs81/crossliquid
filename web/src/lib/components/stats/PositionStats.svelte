...
import { chains, getIcon, vaultChain } from "$lib/wagmi/chains";
import { ChainDisconnectedError, erc20Abi, formatUnits } from "viem";
...
import { positionManagerAbi, positionManagerAddress } from "@generated";
import { getAddressUrl } from "$lib/web3/utils";
import MagnifyigGlassPlusIcon from "phosphor-svelte/lib/MagnifyingGlassPlusIcon";

// Create data for each chain that we're active in.
const launchedChains = chains.filter(
  (chain) => positionManagerAddress[chain.id as keyof typeof positionManagerAddress],
);

const chainList = $derived(
  launchedChains.map((chain) => {
    const chainId = chain.id;
    const managerAddress = deployedContracts.positionManager.deployments[chainId];
    ...
    const usdcAddress = UNIV4_CONTRACTS[chainId as keyof typeof UNIV4_CONTRACTS].usdc as `0x${string}`;
...
<div class="space-y-6">
  {#each chainList as chainData (chainData.chainId)}
    <div class="card bg-base-100 shadow-xl">
      <div class="card-body">
        <h2 class="card-title text-xl">
          <img src={chainData.icon} alt={chainData.name} class="w-10 h-10"> {chainData.name} Manager State
        </h2>

        <h3 class="card-subtitle text-lg">
          Manager Address: {chainData.managerAddress}
          <a
            href={getAddressUrl(chainData.managerAddress, chainData.chainId)}
            target="_blank"
            class="link link-primary"
            title="View on Explorer"
            ><MagnifyigGlassPlusIcon class="w-6 h-6 inline-block" /></a
          >
        </h3>
        <strong>Balances:</strong>
        ETH:
        <QueryRenderer query={chainData.ethBalanceQuery}>
          {#snippet children(data)}
            {formatETH(data.value)}
          {/snippet}
        </QueryRenderer>
        USDC:
        <QueryRenderer query={chainData.usdBalanceQuery}>
          {#snippet children(data)}
            $ {Number(formatUnits(data as bigint, 6)).toFixed(2)}
          {/snippet}
        </QueryRenderer>

        <h3 class="card-subtitle text-lg">Uniswap v4 LP positions:</h3>

        <QueryRenderer query={chainData.positionsQuery}>
          {#snippet children(data)}
                    ...
                    <div class="card-body p-4 gap-3">
                      ...
                      <div class="grid grid-cols-3 gap-4 text-sm">
                        <div>
                          ...
                          <div
                            class="font-medium"
                            title="{formatTokenAmount(currentAmounts.amount0, meta0.decimals, 10)}"
                          >
                            {formatTokenAmount(currentAmounts.amount0, meta0.decimals, 2)}
                          </div>
                        </div>
                        <div>
                          ...
                          <div
                            class="font-medium"
                            title="{formatTokenAmount(currentAmounts.amount1, meta1.decimals, 10)}"
                          >
                            {formatTokenAmount(currentAmounts.amount1, meta1.decimals, 2)}
                          </div>
                        </div>
                        <div>
